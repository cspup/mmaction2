...
from .mobilenet_v3 import InvertedResidual, MobileNetV3
...
class AddLAM(nn.Module):
    def __init__(self,
                 net,
                 n_segment):
        super(AddLAM, self).__init__()
        self.net = net
        self.LAM = LAM(self.net.out_channels, n_segment)

    def forward(self, x):
        return self.LAM(self.net(x))


@MODELS.register_module()
class MobileNetV3LAMME(MobileNetV3):
    def __init__(self,
                 num_segments=8,
                 pretrained2d=True,
                 **kwargs):
    ...
    def init_structure(self):
        ...
        for m in self.modules():
            if isinstance(m, InvertedResidual) and m.with_se:
                print('Replace ME====>')
                # m.se = MELayer(n_segment=self.num_segments)
            if isinstance(m,InvertedResidual) and m.with_res_shortcut:
                print("Add LAM======>")
                m.linear_conv = AddLAM(m.linear_conv, n_segment=self.num_segments)
                # m.linear_conv.add_module('LAM',
                #                          LAM(in_channels=m.linear_conv.out_channels,
